...
3. **Press <kbd>Alt</kbd>+<kbd>F10</kbd>** — `eldenring_2026-08-11_224513.mp4` lands in `Videos\DejaVu`. A balloon confirms. Done.
...
| OBS makes you **remember to start the buffer** | Buffering from the second it launches. Boot, then buffering. |
...
| Discord pings and voice chat **leak into shared clips** | Discord's audio is **excluded from the mix by default**. Zero setup. |
...
| Capture | Auto (follows your game) · any monitor · one window |
| Quality | Low / Medium / High — smart encoding: action gets the bits, menus cost almost nothing |
| Frame rate | 30 to 240 fps, only what your display can actually show |
| System audio | On/off — the mic is **never** recorded |
| Clip folder cap | Off / 10 / 25 / 50 GB |
| Hotkey | Click-to-rebind dialog |
| Corner dot | On/off — click-through, and **never appears in your clips** |

> **Admin games:** if hotkeys die while a game with anti-cheat has focus, hit *Restart as administrator* in the tray once. That's a Windows rule, not ours.
...
<details>
<summary><b>Runs on old PCs — that's the point</b></summary>

<br>

Built for roughly 2015 machines and up:

- **Windows 10 2004+** (the same build provides the Discord-exclusion audio device, so it works everywhere the app runs)
